...
public interface IBranchMapper {
    ...
    // Satılan ve silinen araç stokta değildir; bakım, ekspertiz, rezerve ve transfer stokta sayılır.
    @Mapping(target = "totalCars", expression = "java(branch.getStockItems() != null ? (int) branch.getStockItems().stream().filter(item -> !item.isDeleted() && item.getStatus() != CarStatus.SOLD).count() : 0)")
